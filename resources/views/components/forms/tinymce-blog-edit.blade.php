...
<form action="{{ url('/blog/' . $post->slug) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-6 ">
        <label class="block mx-4 mb-2 text-lg font-medium text-gray-900 dark:text-gray-300" for="default_size">
            Thumbnail
            <span class=" mx-1 text-red-700 text-2xl">
                ✺
            </span>
        </label>
    </div>

    <img class="align-middle rounded-3xl mx-auto" src="{{ asset('images/' . $post->image_path) }}" alt="" width=50%>
    <div class="mb-6 ">
    ...
    <div class="form-group">
        ...
        <textarea id="myeditorinstance" class="block w-full py-5 text-xl bg-transparent border-b-2 outline-none h-60"
            name="description">
    <h2 style="text-align: center;">{{ $post->description }}</h2>
</textarea>
