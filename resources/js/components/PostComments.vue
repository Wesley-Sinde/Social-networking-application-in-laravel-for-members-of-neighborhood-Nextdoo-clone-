<template>
    <div :key="componentKey">
        <div class="flex justify-between mt-3">
            ...
            <label
                v-bind:for="'toggle-' + togleid"
                class="relative inline-flex object-right cursor-pointer right-1 w-fit"
            >
                <input
                    @click="show = !show"
                    type="checkbox"
                    value=""
                    v-bind:id="'toggle-' + togleid"
                    class="sr-only peer"
                />
                <!-- <div
        ...
        <div v-if="show">
            ...
            <!-- <createomments-component :postid="postidtosend" :user_id="user_id" /> -->
            <div
      ...
      <div class="text-red" slot="no-results">No more users</div> -->
                ...
                <template v-slot="{ state, error, auto, direction }">
                    ...
                    <template v-else-if="state === 'error'"
                        >Error: {{ error.message }}. Click to retry</template
                    >

                    <template
                        v-else-if="state === 'standby' && auto !== 'in-advance'"
                    >
                        {{
                            auto
...
export default {
    props: ["postid", "user_id", "view_count"],
    // mounted() {
    ...
    methods: {
        ...
        handleLoadMore($state) {
            this.$http
                .get("/PostComments/" + this.postid + "?page=" + this.page)
                .then((res) => {
                    return res.json();
                })
                .then((res) => {
